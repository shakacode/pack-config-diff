...
export class DiffFormatter {
  ...
  formatContextual(result: DiffResult): string {
    ...
    sortedEntries.forEach((entry) => {
      ...
      lines.push(this.formatContextualEntry(entry, displayIndex, leftLabel, rightLabel));
      lines.push("");
      displayIndex += 1;
    });
  ...
  private getShortLabel(filename: string | undefined, fallback: string): string {
    ...
    const match = withoutExt.match(/(development|production|test|dev|prod).*?(client|server)/i);
    if (match) {
      const env = match[1]
        .toLowerCase()
    ...
    if (envMatch) {
      return envMatch[1].toLowerCase().replace("development", "dev").replace("production", "prod");
    }
  ...
  formatMarkdown(result: DiffResult): string {
    ...
    if (result.metadata.leftFile && result.metadata.rightFile) {
      lines.push(`Comparing \`${result.metadata.leftFile}\` vs \`${result.metadata.rightFile}\``);
      lines.push("");
    }
    ...
    lines.push(`| # | Op | Path | ${leftLabel} | ${rightLabel} |`);
    lines.push("| --- | --- | --- | --- | --- |");
    ...
    for (const entry of sortedEntries) {
      ...
      const symbol = entry.operation === "added" ? "+" : entry.operation === "removed" ? "-" : "~";

      const leftValue =
        entry.operation === "added" ? "<not set>" : this.formatValue(entry.oldValue);
      const rightValue =
        entry.operation === "removed" ? "<not set>" : this.formatValue(entry.newValue);
  ...
  private getDocumentationForPath(pathSegments: string[]) {
    ...
    const normalizedSegments = pathSegments.filter((segment) => !/^\[\d+\]$/.test(segment));
  ...
  private groupByOperation(entries: DiffEntry[]): Record<DiffOperation, DiffEntry[]> {
