...
describe("FileWriter", () => {
  ...
  describe("generateFilename", () => {
    ...
    test("JSON format uses .json extension", () => {
      const name = FileWriter.generateFilename(
        "rspack", "development", "server", "json"
      )
    ...
    test("inspect format uses .txt extension", () => {
      const name = FileWriter.generateFilename(
        "webpack", "production", "client", "inspect"
      )
    ...
    test("buildName overrides env in filename", () => {
      const name = FileWriter.generateFilename(
        "webpack", "production", "client", "yaml", "my-build"
      )
  ...
  describe("writeMultipleFiles", () => {
    test("writes all output files to directory", () => {
      ...
      expect(
        fs.readFileSync(path.join(outputDir, "webpack-prod-client.yml"), "utf8")
      ).toBe("mode: production\n")
      expect(
        fs.readFileSync(path.join(outputDir, "webpack-prod-server.yml"), "utf8")
      ).toBe("mode: production\ntarget: node\n")
    })
    ...
    test("logs created file paths", () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining("[pack-config-diff] Created:")
      )
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining("Exported 1 config file(s)")
      )
    })
    ...
    test("strips path traversal from filenames", () => {
      ...
      expect(
        fs.existsSync(path.join(outputDir, "evil.yml"))
      ).toBe(true)
      expect(
        fs.existsSync("/etc/evil.yml")
      ).toBe(false)
    })
  })
})
